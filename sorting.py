...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    This _merged function could be implemented as a
    local function within the merge_sorted scope rather
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        xs_eq = [x for x in xs if x == pivot]
...
def quick_sort(xs, cmp=cmp_standard):
    ...
    that it will no longer be a
    [stable sort](https://en.wikipedia.org/wiki/Sorting_algorithm#Stability),
    but this is typically inconsequential.

    Follow the pseudocode of the Lomuto
    partition scheme given on wikipedia
    ...
    def partition(xs, lo, hi):
        ...
        for j in range(lo, hi):
            if xs[j] == pivot or cmp(xs[j], pivot) == -1:
                i += 1
                xs[i], xs[j] = xs[j], xs[i]
        i += 1
